        ...
        <section className='pt-8'>
            <div className='container'>
                <div className='flex items-center justify-between'>
                    <h2 className='font-medium text-lg tracking-tight'>Популярные категории</h2>
                ...
                <div className='pt-4 grid grid-cols-2 gap-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-7'>
                    {Array.from({ length: 7 }, (_, i) => i + 1).map((index) => {
                        const gradient = softGradients[Math.floor(Math.random() * softGradients.length)];

                        return <a href='/products' style={{ background: gradient }} className='relative group flex h-56 items-end p-4 rounded-lg overflow-hidden hover:ring-2 hover:ring-gray-200' key={index}>
                            <div className='flex justify-center absolute inset-0 brightness-75 group-hover:brightness-50'>
                                <img src="https://picsum.photos/800/600" className='w-full object-cover' alt="airpods" />
                            </div>
                            <p className='leading-4 pt-3 text-xl font-medium tracking-tight relative text-white'>{categories[index]}</p>
                        </a>
                    })}
